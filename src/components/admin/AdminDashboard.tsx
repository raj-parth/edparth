...
export const AdminDashboard: React.FC = () => {
  ...
  // Storage Upload States (Default to Telegram/Drive Link for 100% Free Unlimited Storage)
            ...
            <form onSubmit={handleUploadContent} className="space-y-3.5">
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-xs font-bold text-slate-700">PDF Document / Notes Source *</label>
                  <span className="text-[10px] font-mono text-emerald-700 bg-emerald-50 border border-emerald-200/60 px-2 py-0.5 rounded-full font-bold">
                    100% Free & Unlimited
                  </span>
                </div>
                ...
                {uploadMethod === 'link' ? (
                  <div className="space-y-2">
                    <input
                      ...
                      placeholder="e.g. https://t.me/edparthbooks or Google Drive Link"
                      className="w-full bg-[#f8fafc] border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-800 focus:outline-none focus:border-indigo-600"
                    />
                    <div className="flex items-center gap-2 flex-wrap">
                      <button
                        type="button"
                        onClick={() => {
                          setNewFileUrl('https://t.me/edparthbooks');
                          if (!newFileSize) setNewFileSize('Official Telegram Hub');
                        }}
                        className="px-2.5 py-1 rounded-lg bg-sky-50 hover:bg-sky-100 text-sky-700 border border-sky-200 text-[10px] font-bold cursor-pointer transition-colors flex items-center gap-1"
                      >
                        <span>📢 Insert Official Telegram Link</span>
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="border-2 border-dashed border-slate-200 hover:border-indigo-400 bg-slate-50/60 rounded-2xl p-4 text-center transition-all relative">
                    ...
                    {isUploadingFile ? (
                      <div className="space-y-2 py-2">
                        ...
                        <p className="text-xs font-bold text-slate-800">Uploading to Cloud Storage...</p>
                      ...
                      <div className="space-y-1.5 py-1 text-left bg-emerald-50 border border-emerald-200 rounded-xl p-3">
                        <div className="flex items-center gap-2 text-emerald-700">
                          ...
                          <p className="text-xs font-bold truncate">File Ready: {uploadedFileName || 'Link configured'}</p>
                        </div>
                        <p className="text-[10px] text-emerald-600 font-mono truncate">{newFileUrl}</p>
                        <p className="text-[10px] font-bold text-slate-500">Size: {newFileSize} • Click to replace</p>
                      </div>
                    ) : (
